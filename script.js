...
function startGame(type) {
    startTimer(); // Start the RTA timer immediately!

    if (type === 'basic') {
        runStandardGame(basicElements, 10, '基礎25 RTA');
    } else if (type === 'master') {
        runStandardGame(allElements, 10, 'ランダム10 RTA');
    ...
    function nextQuestion() {
        ...
        options.forEach(opt => {
            ...
            btn.onclick = () => {
                ...
                count++;
                setTimeout(nextQuestion, 800); // Faster transition for RTA
            };
...
// --- GAME ENGINE 2: SURVIVAL 118 RTA ---
    ...
    function showNext() {
        ...
        options.forEach(opt => {
            ...
            btn.onclick = () => {
                ...
                currentIdx++;
                setTimeout(showNext, 600); // Super fast for marathon
            };
            container.appendChild(btn);
        });
    }
